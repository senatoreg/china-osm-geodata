...
def to_country(x):
    if isinstance(x, str):
        return countryconvertion[x] if x in countryconvertion else \
            unicodedata.normalize('NFD', x).encode("ascii", "ignore").decode()


def to_year(x):
    return dt.datetime.strptime(str(int(
        float(x))), '%Y') if x else float('NaN')
...
countryconvertion = {
    ...
    "Democratic People's Republic of Korea": "Korea (Democratic People's "
    "Republic of)",
...
class CountryCode():
    def __init__(self, filename):
        ...
        for c in self._data:
            c["name"] = unicodedata.normalize(
                'NFD', c["name"]).encode("ascii", "ignore").decode()
            # print(c["name"])
    ...
    def reverse_lookup(self, x):
        if pd.isna(x):
            return [float('NaN')]
        for c in self._data:
            if c["name"] == unicodedata.normalize(
                    'NFD', x).encode("ascii", "ignore").decode():
                return c["alpha-3"]
        raise KeyError("{x} not found".format(x=x))
        # return float('NaN')


def row_to_geojson(x, aggr_by, group_by, value,
                   filled, opacity, cumulative, registry):
    long = x['centroid_longitude']
    lat = x['centroid_latitude']
    shapeId = x['shapeID']
    shapeGrp = x['shapeGroup']
    val = (x[value['name']] - value['min']) / (value['max'] - value['min'])
    idxVal = dt.datetime.strftime(x[aggr_by], "%Y") if not isinstance(
        x[aggr_by], type(pd.NaT)) else float('NaN')
    if (long, lat, shapeId, shapeGrp) in registry.keys():
        logger.debug('Found duplicate: longitude= ' + str(long) + ', lat= '
                     + str(lat) + ', value= ' + str(val))
        prev = registry[(long, lat, shapeId, shapeGrp)]
        last_idx = int(prev['properties']['indexValue'][-1])
        for i in range(last_idx + 1, int(idxVal)):
            logger.debug('Add index= ' + str(i) + ' for ' + str(long) + ', '
                         + str(lat))
            prev['properties']['indexValue'].append(str(i))
        prev['properties']['transient'] = True
    feature = {'type': 'Feature', 'properties': {}}
    feature['properties']['opacity'] = opacity
    feature['properties']['indexValue'] = [idxVal]
    feature['properties']['group'] = x[group_by]
    feature['properties']['transient'] = True if not cumulative else False
    ...
    registry[(long, lat, shapeId, shapeGrp)] = feature
...
def main(argv):
    ...
    parser.add_argument("--cumulative", "-c", dest="cumulative",
                        action="store_true",
                        help="define if cumsum aggregate values")
    parser.add_argument("--projects", "-p", dest="projects",
                        action="store_true",
                        help="define if add geojson data of each project")
    parser.add_argument("--adm-loc-files", "-a", dest="adm_loc_files",
                        action="store", nargs="+",
    ...
    parser.add_argument("--aggr-by", "-m", dest="aggr_by", action="store",
                        ...
                        help="column to aggregate data for index")
    parser.add_argument("--group-by", "-g", dest="group_by", action="store",
                        default="Flow Type", required=False,
                        type=str, help="column to categorize data")
    parser.add_argument("--fill-by", "-f", dest="fill_by", action="store",
                        nargs=2, default=["Status", "Completion"],
                        required=False, type=str, help="column that defined "
                        "if data geojson point should be filled "
                        "(default:  'Status' 'Completion')")
    parser.add_argument("--value-column", "-v", dest="value_column",
                        action="store", default=GEOPANDAS_VALUE_COLUMN,
                        required=False, type=str,
                        help="Metric to index aggregated data")
    ...
    parser.add_argument("--iso-3166", "-i", dest="isoa3db", action="store",
                        default="iso_a3.json", required=False, type=str,
                        help="iso alpha-3 country code database json file")
    ...
    new_g = df.groupby(by=["recipient_code", aggr_by],
                       as_index=False)[value_column].sum()
    new_g[aggr_by] = new_g[aggr_by].map(lambda x: dt.datetime.strftime(x, "%Y"))
    new_g = new_g.pivot(columns="recipient_code", index=aggr_by,
                        values=value_column).fillna(0)
    if args.cumulative:
    ...
    if args.projects:
        geojson_dfs = []
        for __loc_filename in tqdm(args.adm_loc_files,
                                   desc="Processing ADM location files",
                                   leave=False):
            geojson_df = pd.read_csv(__loc_filename, dtype=gcdf30_adm_headers,
                                     converters=gcdf30_converters)
            geojson_df = geojson_df.merge(df, left_on="id",
                                          right_on="aiddata_record_id")
            geojson_df = geojson_df[geojson_df[value_column].isna() == False]
            geojson_df = geojson_df.sort_values([aggr_by], ascending=[True])
            if args.cumulative:
                geojson_df[value_column] = geojson_df.groupby(
                    by=['centroid_longitude', 'centroid_latitude',
                        'shapeID', 'shapeGroup'],
                    as_index=False)[value_column].cumsum(numeric_only=True)
            geojson_value = {
                "name": value_column,
            ...
            registry = {}
            geojson_df["feature"] = geojson_df.apply(
                lambda x: row_to_geojson(x, aggr_by, group_by, geojson_value,
                                         fill_by, 0.33, args.cumulative,
                                         registry),
                axis=1)
